...
def get_P_BLB(tmp_lane,calib_cam,calib_cam2Lidar):
    ...
    Pc_lane = np.array([[(X_c - cx)/fx, (Y_c - cy)/fy, 1]]).reshape(3,1)

    Pw_lane = R.T@(Pc_lane - t)   ### Pw_lane = R@Pc_lane + t ## 이 아닌가..?
    # Pw_lane = R@Pc_lane + t ## 이 아닌가..?

    Cc = np.array([0,0,0]).reshape(3,1)
    Cw = R.T@(Cc - t)

    k = -Cw[2]/(Pw_lane[2]-Cw[2])

    P = (Cw+k*(Pw_lane-Cw)).T[0]

    # Pc_lane = np.array([[1, -(X_c - u0)/fx, -(Y_c - v0)/fy]]).reshape(3,1)
...
def get_wline(tmp_txt_files, wline):
    for txt_file in tmp_txt_files:
        ...
        for tmp_line in line :
            ...
            wline = np.vstack((wline,P_PLB[:2])) if (len(wline)==0)==False else P_PLB[:2]                            
    ...
    for tmp_path_1 in first_tqdm:
        ...
        for tmp_path_2 in second_tqdm:
            ...
            for tmp_txt_files in third_tqdm:
                ...
                plt.plot(wline[:,0]*WXCONST,wline[:,1],'*',color='red',markersize=3)
                plt.xlabel('x')
                plt.ylabel('y')
                ...
                ## save wline txt file
                np.savetxt(os.path.join(os.path.dirname(img_path), tmp_txt_files[0][:-6] + ".txt"), wline, fmt='%.6f', delimiter=',')
