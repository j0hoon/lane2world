...
if __name__ == "__main__":
    ...
    for tmp_path_1 in first_tqdm:
        ...
        for tmp_path_2 in second_tqdm:
            ...
            txt_path = os.path.join(os.getcwd() +r"\world_line_text", path.split('\\')[-2] ,path.split('\\')[-1], img_files[0])
            ...
            for tmp_txt_files in third_tqdm:
                ...
                plt.plot(wline[:,1],wline[:,0],'.',color='red',markersize=1)
                plt.xlabel('y')
                plt.ylabel('x')
                plt.xlim(5,-5) ## 실제로는 y축
                plt.ylim(-10,20) ## 실제로는 x축
                ...
                tmp_txt_name = os.path.join(os.path.dirname(txt_path), tmp_txt_files[0][:-6] + ".txt")
                file=open('tmp_txt_name','wb')
                file.write(wline)
                file.close() 
